...
void parse_obj(const char* path, float** vb, float** tex, int *stride, size_t *num_triangles)
{
	...
	const char* data = get_file_data(&data_len, path);
	if (data == NULL) {
		exit(-1);
		/* return 0; */
	}
	...
	*vb = (float*)malloc(sizeof(float) * (*stride) * (*num_triangles) * 3);
	if (!(*vb)) {
		printf("failed to allocate vb\n");
	}

	if (tex) {
		*tex = (float*)malloc(sizeof(float) * (*stride) * (*num_triangles) * 2);
		*tex = attrib.texcoords;
	}
	...
	// DO THIS AFTER STOREING IN BUFFER
	// free(vb);
